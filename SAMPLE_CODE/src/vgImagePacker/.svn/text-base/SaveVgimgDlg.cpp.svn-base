...
void CSaveVgimgDlg::OnBnClickedSavevgimgAimfileOpen()
{
	// TODO: Add your control notification handler code here
	char szFilters[]=
		"images list Files (*.vgi)|*.vgi|All Files (*.*)|*.*||";

	CFileDialog fileDlg (false, "vgi", "*.vgi",
		OFN_FILEMUSTEXIST| OFN_HIDEREADONLY, szFilters, this);
...
void CSaveVgimgDlg::OnBnClickedDdsSave()
{
	...
	vgCore::StringPairList::iterator iter = pResult->begin();
	vgCore::StringPairList::iterator iter_end = pResult->end();
	...
	for ( ; iter != iter_end ; ++iter )
	{
		...
		vgCore::StreamReaderPtr preader = 
			vgCore::StreamFactory::createReaderFromFile( src );

		assert( preader.isNull() == false );

		bool ret = vgMesh::FileWriterVgm::writeToVgmFileFromModStream(
			preader , dst , _lod0_factor , _lod1_factor );
