...
#define EFFECT_NAME_LENGTH	128
	...
	//----------------------------------------------------------------
	GlassNode::GlassNode(int nVersionMod /*= VERSION_1*/)
		:ObjectNode(nVersionMod)
	{
		//_rendererType = RENDERER_TYPE_GPUSPECIAL_FLOOR;
		setDefault();

		this->setRenderingPriority( -100 );
	}
	//----------------------------------------------------------------
	GlassNode::~GlassNode()
	{
	...
	//----------------------------------------------------------------
	void GlassNode::WriteNodeToVG( CFile &fp )
	{
	...
	//----------------------------------------------------------------
	unsigned long GlassNode::PositDataToNode( char *data )
	{
		unsigned long pos = 0;
	...
	//----------------------------------------------------------------
	bool GlassNode::recreate( vgCam::CamManager *camera , const vgCore::StringVector& strvec , 
	...
	//----------------------------------------------------------------
	void GlassNode::assignTexture( int i )
	{
		if ( pMats[i].pTexture.isNull() == false )
		{
			pMats[i].pTexture->useBind();
	...
	//----------------------------------------------------------------
	vgKernel::StringVector GlassNode::getTexNameVec()
	{
