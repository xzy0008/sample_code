...
namespace	vgGroup
{
	...
	void GroupNodeItem::erase( String node_name, GroupNodeItemPtr pNode, bool bRecursive /*= false */ )
	{
			...
			itr != pSubItemsTemp->end(); itr ++ )
		{
			...
			vgKernel::StringUtility::toLowerCase(nodeDes);
		...
		const vgKernel::StringVector& filters, FolderNameMap folderMap )
	{
		...
		for ( ; itr != pSubItems->end(); )
		{
			...
			if (  itr->isLeafItem() )
			{
				...
				vgKernel::StringUtility::toLowerCase(nodeName);

				// 是否符合过滤条件

				vgKernel::StringVector::const_iterator itrFilter = filters.begin();
				for( ; itrFilter != filters.end(); itrFilter ++ )
				{
					String filterString(*itrFilter);
					vgKernel::StringUtility::toLowerCase(filterString);

					//过滤第三方库和标准库的debug版
					if ( filters.size() == 1 && filterString == "debug" )
					{
						std::string ext = vgKernel::StringUtility::getFileExtension( nodeName );
						if ( ext == "dll" )
						{
	...
	GroupNodeItem* GroupNodeManager::getVersionByVersionName( String& name )
	{
		...
		vgKernel::StringUtility::toLowerCase( nameLower );
	...
	void GroupNodeManager::initializeGroupNode()
	{
		...
		/*_versions.insert( VersionPair("debug",	rootNode ) );
		_versions.insert( VersionPair("release",rootNode ) );
		_versions.insert( VersionPair("common",	rootNode ) );*/
		...
		_filters.insert( FiltersPair("debug",	filterString ) );

		filterString.clear();
		filterString.push_back("debug");
		_filters.insert( FiltersPair("release",	filterString ) );

		filterString.clear();
		filterString.push_back("debug");
		filterString.push_back("release");
		_filters.insert( FiltersPair("common",	filterString ) );*/

	}

	//----------------------------------------------------------------
	vgKernel::StringVector* GroupNodeManager::getFilterByVersionName( String& name )
	{
		String nameLower(name);
