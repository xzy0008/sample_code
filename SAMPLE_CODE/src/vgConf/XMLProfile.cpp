...
BOOL CXmlProfile::Load(LPCTSTR lpszFileName, ENCODING eEncoding /*= Default*/)
{
	...
	if (xml.Load(lpszFileName, (CXmlParser::ENCODING)eEncoding))
	{
		...
		{
			TRACE(_T("pRoot=<%s>\n"), pRoot->m_strName);
			assert(pRoot->m_strName == m_strProfileTag);
			if (pRoot->m_strName == m_strProfileTag)
			{
				...
				while (pSection)
				{
					...
					{
						CWTLString strSectionName = pSection->GetValue(m_strNameTag);
						if (!strSectionName.IsEmpty())
						{
							CWTLString strSectionReadOnly = pSection->GetValue(m_strReadOnlyTag);
							...
							while (pKey)
							{
								...
								{
									CWTLString strKeyName = pKey->GetValue(m_strNameTag);
									CWTLString strKeyValue = pKey->GetValue(m_strValueTag);
									CWTLString strKeyReadOnly = pKey->GetValue(m_strReadOnlyTag);
...
BOOL CXmlProfile::Save(LPCTSTR lpszFileName, ENCODING eEncoding /*= Default*/)
{
	...
	//edit by KingHj 2010.03.18    debug版转DDS工具，调试的时候在此处崩溃
	...
	if (lpszFileName && (lpszFileName[0] != _T('\0')))
	{
		...
		else
		{
			...
			//	bFileIsUnicode ? 16 : 8);
			str.Format(_T("<?xml version=\"1.0\" encoding=\"GBK\"?>\r\n") );
			...
			CWTLString strSection = _T("");
			CWTLString strKey = _T("");
...
//
// Returns:     CWTLString      - string containing value
//
CWTLString CXmlProfile::GetKey(LPCTSTR lpszSectionName, 
							   LPCTSTR lpszKeyName, 
							   LPCTSTR lpszDefault)
{
	CWTLString strResult(lpszDefault);
								   ...
								   UINT* pBytes)
{
	...
	*pBytes = 0;
	...
	CWTLString str = GetProfileString(lpszSection, lpszEntry, NULL);
	if (str.IsEmpty())
		return FALSE;
...
//                            entry cannot be found
//
// Returns:     CWTLString - the string that is stored in lpszEntry, or 
//                           lpszDefault if lpszEntry is not found
//
CWTLString CXmlProfile::GetProfileString(LPCTSTR lpszSection, 
										 LPCTSTR lpszEntry,
										 LPCTSTR lpszDefault)
{
	_ASSERTE(lpszSection);
