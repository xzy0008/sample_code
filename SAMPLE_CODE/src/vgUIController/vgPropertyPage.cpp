...
void vgPropertyPage::ConnectNode(vgBaseEntry* node, PropertiesParamVector* propParam)
{
	CBCGPProp*	pGroup = NULL;
	CBCGPProp*  prop = NULL;
	CBCGPProp*	option = NULL;
	...
	if (! m_useVectorParam)
	{
		...
		for (int i=0; i<m_numOfParam; i++)
		{
			if(m_pPropParam[i].label == name)
			{
				...
				vgUI::UIController::getSingleton().GetCurrentSelectedNode()->OnPropertyChanged(name);
				return NULL;
			}
		}
	}
