...
#include "vgUIcontroller/vgCursorDef.h"
	...
	: m_pDragData(0)
{
	...
	m_Commandmap[NID_RENAME]		        = &CEditTreeCtrl::DoEditLabel;
	m_Commandmap[NID_ADD_CHILD]          = &CEditTreeCtrl::DoInsertChild;
	m_Commandmap[NID_ADD_ROOT]           = &CEditTreeCtrl::DoInsertRoot;
	m_Commandmap[NID_ADD_SIBLING]        = &CEditTreeCtrl::DoInsertSibling;
	m_Commandmap[NID_DELETE]             = &CEditTreeCtrl::DoDeleteItem;
	m_Commandmap[NID_SORT_LEVEL]         = &CEditTreeCtrl::DoSortCurrentLevel;
	m_Commandmap[NID_SORT_LEVELANDBELOW] = &CEditTreeCtrl::DoSortCurrentLevelAndBelow;
}
...
bool CEditTreeCtrl::DoDeleteItem(HTREEITEM hItem) {
	...
	AfxMessageBox("Delete item here");
...
void CEditTreeCtrl::DragEnd() {
	...
	if(m_pDragData->IsRightDragging()) {
		...
		bool bCtrl = (::GetKeyState(VK_CONTROL) & 0x8000) != 0;
		//DragMoveItem(m_pDragData->GetDragItem(), hDrop, eHint, false);
		DragStop();
	}
}
...
void CEditTreeCtrl::DragDisplayContextMenu(CPoint & point) {
	...
	VERIFY(menu.AppendMenu(MF_STRING, NID_DRAG_COPY, _T("Copy Here")));
	VERIFY(menu.AppendMenu(MF_STRING, NID_DRAG_MOVE, _T("Move Here")));
	VERIFY(menu.AppendMenu(MF_SEPARATOR));
	VERIFY(menu.AppendMenu(MF_STRING, NID_DRAG_CANCEL, _T("Cancel")));

	bool bCtrl = (::GetKeyState(VK_CONTROL) & 0x8000) != 0;
	menu.SetDefaultItem(bCtrl ? NID_DRAG_COPY : NID_DRAG_MOVE);
...
void CEditTreeCtrl::DisplayContextMenu(CPoint & point) {
	...
	if(bOnItem) {
		if(CanInsertItem(GetParentItem(hItem)))
			VERIFY(add.AppendMenu(MF_STRING, NID_ADD_SIBLING, _T("New Sibling\tINS")));
		if(CanInsertItem(hItem))
			VERIFY(add.AppendMenu(MF_STRING, NID_ADD_CHILD, _T("New Child Item\tCtrl+INS")));
	}
	if(CanInsertItem(0))
		VERIFY(add.AppendMenu(MF_STRING, NID_ADD_ROOT, _T("New Root Item\tShift+INS")));

	CMenu sort;
	VERIFY(sort.CreatePopupMenu());
	VERIFY(sort.AppendMenu(MF_STRING, NID_SORT_LEVEL, _T("Current Level\tCtrl+S")));
	VERIFY(sort.AppendMenu(MF_STRING, NID_SORT_LEVELANDBELOW, _T("Current Level And Below\tCtrl+Shift+S")));

	CMenu menu;
	VERIFY(menu.CreatePopupMenu());
	if(bOnItem) {
		if(CanEditLabel(hItem))
			VERIFY(menu.AppendMenu(MF_STRING, NID_RENAME, _T("Rename\tF2")));
		if(CanDeleteItem(hItem))
			VERIFY(menu.AppendMenu(MF_STRING, NID_DELETE, _T("Delete\tDEL")));
	}
...
BEGIN_MESSAGE_MAP(CEditTreeCtrl, CTreeCtrl)
	...
	//}}AFX_MSG_MAP
	ON_COMMAND_RANGE(NID_RENAME, NID_MESH_MAX_CMD-1, OnContextCmd)
	ON_COMMAND_RANGE(NID_DRAG_COPY, NID_MAX_DRH-1, OnDragContextCmd)
END_MESSAGE_MAP()
...
void CEditTreeCtrl::OnDragContextCmd(UINT id) {
	...
	if(id != NID_DRAG_CANCEL) {
		bool bCopy = (id == NID_DRAG_COPY);
