...
bool CUnrarDLL::OpenRAR(char* rarTo)
{
	...
	(FARPROC&)OpenArchiveEx = GetProcAddress(unrarDLL, _T("RAROpenArchiveEx"));
	(FARPROC&)CloseArchive = GetProcAddress(unrarDLL, _T("RARCloseArchive"));
	(FARPROC&)ProcessRARFile = GetProcAddress(unrarDLL, _T("RARProcessFile"));
	(FARPROC&)ReadRARHeaderEx = GetProcAddress(unrarDLL, _T("RARReadHeaderEx"));
	...
	while ((readHeaderCode = ReadRARHeaderEx(archiveHandle, &headerData)) == 0)
	{
		// Get the file size
		...
		if ((processFileCode = ProcessRARFile(archiveHandle, 0, NULL, NULL)) != 0)
		{
			break;
		}
	}
...
bool CUnrarDLL::UnRARArchive()
{
	...
	(FARPROC&)CloseArchive = GetProcAddress(unrarDLL, _T("RARCloseArchive"));
	(FARPROC&)ProcessRARFile = GetProcAddress(unrarDLL, _T("RARProcessFile"));
	(FARPROC&)ReadRARHeader = GetProcAddress(unrarDLL, _T("RARReadHeader"));
	...
	while ((readHeaderCode = ReadRARHeader(archiveHandle, &headerData)) == 0)
	{
		...
		// UnRAR this file
		processFileCode = ProcessRARFile(archiveHandle, 2, unrarDir.GetBuffer(unrarDir.GetLength()), NULL);
