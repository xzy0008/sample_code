...
namespace
{
	...
	/** 
	*	SuicideException used by the suicide pill to break out of the initialize loop
	*/
														...
														threads(threadSize)
{
	...
	isSingleThreaded = ( threadSize == 0 );
	threadPoolHandle = NULL;
}
/// Static function to fire off all of the initialize handlers for each thread
UINT  AFX_CDECL ThreadPool::internalThreadProc(LPVOID lpParam)
{
	...
	try
	{
		data->threadPool->acceptHandler(data->threadId);			// And start this threads initialize handler.
	}
	catch(...)
	{
	}
...
void ThreadPool::acceptHandler(unsigned int threadId) throw()
{
	...
	for(;;)
	{
		...
		try
		{
			...
			if(!jobQueue.empty())
			{
				// 注意在这里request指针被复制了一份,于是脱离作用域时自动执行delete
				vgKernel::SharePtr<ThreadRequest> request(jobQueue.front());		// Autopointer takes ownership of request
				jobQueue.pop();
...
int ThreadPool::initialize()
{
	...
	if(!alive)						// Cannot initialize twice!
	{
		for(int i = 0 ; i < threads ; i++)
		{
			try
			{
				...
				handle = 
					AfxBeginThread( internalThreadProc, data ,
		...
		std::vector<CWinThread*>::iterator iter = pool.begin();
		std::vector<CWinThread*>::iterator iter_end = pool.end();

		for ( ; iter != iter_end ; ++ iter )
		{
			(*iter)->ResumeThread();
		}
		//std::for_each(pool.begin(), pool.end(), ::ResumeThread);
		alive=true;		
	}
...
void ThreadPool::submitRequest(ThreadRequest *request)
{
	...
	::WaitForSingleObject(notFull.getHandle(), INFINITE);
	try
	{
...
{
	for(std::vector<CWinThread*>::const_iterator j = pool.begin() ; j != pool.end() ; ++j)
		this->submitRequest(new SuicidePill());			// Feed thread a suicide pill
	alive = false;			// Stop the queue receiving

	for(std::vector<CWinThread*>::const_iterator i = pool.begin() ; i != pool.end() ; ++i)
	{
		switch(::WaitForSingleObject( (*i)->m_hThread , 10000))
		{
		case WAIT_OBJECT_0:
...
unsigned int __stdcall ThreadPool::threadPoolThreadFunc( void* lpParam )
{
	assert(0);
	//while ( true )
	//{
	//	switch(::WaitForSingleObject(, 10000))
	//	{
	//	case WAIT_OBJECT_0:
	//		// This is what we WANT to happen.
	//		break;
