...
CProgressST::CProgressST()
{
	...
	m_nStep = 2;
	pDCBack = new CDC;
//  	bmpBack.LoadBitmap(IDB_SPLASH);
// 	bmpBack = (HBITMAP)LoadImage(AfxGetInstanceHandle(),"res\\BACK.BMP",IMAGE_BITMAP,0,0,LR_LOADFROMFILE);
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if (ctrlRect.Width()>0)
	{
		pDC->BitBlt(ctrlRect.Width(),0,m_rect.Width(),m_rect.Height()+2,pDCBack,ctrlRect.Width(),0,SRCCOPY);


	} 
	else
	{
		pDC->BitBlt(0,0,m_rect.Width()+(0-ctrlRect.Width()),m_rect.Height()+2,pDCBack,0,0,SRCCOPY);
	...
	// Copy from temporary DC to screen (only the percentage rectangle)
 	if (ctrlRect.IsRectEmpty() == FALSE)
 		pDC->BitBlt(0, 0, ctrlRect.Width(), ctrlRect.Height()+2, &tempDC, 0, 0, SRCCOPY);
