...
BOOL CFileTreeCtrl::SetItemState(HTREEITEM hItem, int newState, int nAction)
{
	...
	if (newState != REFRESH && newState != UNCHECKED && newState != UNCHECKED_REAL 
		...
		&& newState != FILE_CHECKSTATE && newState != CHILD_FILE_FOLDER && newState != CHILD_ALLFILES_FOLDER 
	...
	if (fInfo && fInfo->IsFolder && newState == FILE_CHECKSTATE) return FALSE;
	if (fInfo && !fInfo->IsFolder && newState != FILE_CHECKSTATE && newState != UNCHECKED && newState != DISABLED && newState !=REFRESH) return FALSE;
	...
	if (fInfo) 
	{
		...
		if (!fInfo->IsFolder) 
		{
			if (newState==FILE_CHECKSTATE) SelFiles.SetAt(hItem,*(fInfo->FullName));
			...
			while (hChild)
			{
				...
				if (ChildState!=DISABLED && ChildState!=NOSTATE) 
				{
					...
					case CHILD_ALLFILES:
						...
						else SetItemState(hChild, FILE_CHECKSTATE); break;
					case CHILD_NOFILE_ALLFOLDERS:
				...
				if (newState == REFRESH) 
				{
					...
					{
						fInfo->nNumFiles++;
						if (ChildState == FILE_CHECKSTATE) fInfo->nSelFiles++;
					}
				}
...
Quit1:
		...
		if (nAction == PARENTS || nAction == ALL)
		{
			...
			if (newState == DISABLED) // Folder is disabled
			{
				...
				{
					lptvid->nNumFiles--;
					if (oldState == FILE_CHECKSTATE) lptvid->nSelFiles--;
				}
				else 
				{
					lptvid->nNumDirs--;
			...
			{
				// File selected
				if (newState == FILE_CHECKSTATE) lptvid->nSelFiles++;
...
BOOL CFileTreeCtrl::CheckItem(HTREEITEM hItem)
{
	...
	if(nState == FILE_CHECKSTATE || nState == CHILD_ALL) return TRUE;
...
BOOL CFileTreeCtrl::ToggleForward (HTREEITEM hItem)
{
	...
	if (nState == FILE_CHECKSTATE) return UncheckItem(hItem);
