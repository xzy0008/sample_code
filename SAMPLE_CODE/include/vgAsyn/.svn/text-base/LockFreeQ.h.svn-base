...
namespace LockFreeTech 
{
	...
	public:	
		...
		void enqueue(const T& t)
		{
			...
			{
				// Read _tail.ptr and _tail.count together
				...
				if(tail.count == _tail.count && tail.ptr == _tail.ptr)
				{
					if(NULL == next.ptr) // Was _tail pointing to the last node?
					{
					...
					else // _tail was not pointing to the last node
					{
						// Try to swing _tail to the next node
						CAS(_tail, tail, pointer_t(next.ptr,tail.count+1) );
					}
		...
		bool dequeue(T& t)
		{
			...
			while(bDequeNotDone)
			{
				...
				if(head.count == _head.count && head.ptr == _head.ptr)
				{
					if(head.ptr == tail.ptr) // is tail falling behind?
					{
						...
						CAS(_tail,tail, pointer_t(next.ptr,tail.count+1)); // _tail is falling behind. Try to advance it
					} // endif
					...
					else // no need to deal with tail
					{
						...
						if(CAS(_head,head, pointer_t(next.ptr,head.count+1) ) )
						{
							bDequeNotDone = false;
						}
					}
